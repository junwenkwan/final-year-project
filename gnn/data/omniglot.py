...
class Omniglot(data.Dataset):
    ...
    def _init_folders_(self):
        decompress = False
        if not os.path.exists(os.path.join('./', 'compacted_datasets')):
            os.makedirs(os.path.join('./', 'compacted_datasets'))
            decompress = True
        return decompress

    def check_decompress(self):
        return os.listdir('%s' % self.root) == []

    def _check_exists_(self):
        return os.path.exists(os.path.join('./', 'compacted_datasets', 'omniglot_train.pickle')) and \
               os.path.exists(os.path.join('./', 'compacted_datasets', 'omniglot_test.pickle'))

    def _preprocess_(self):
        print('\nPreprocessing Omniglot images...')
        (class_names_train, images_path_train) = parser.get_image_paths(os.path.join(self.root))
        (class_names_test, images_path_test) = parser.get_image_paths(os.path.join(self.root))
        ...
        with open(os.path.join('./', 'compacted_datasets', 'omniglot_train.pickle'), 'wb') as handle:
            pickle.dump(train_set, handle, protocol=2)
        with open(os.path.join('./', 'compacted_datasets', 'omniglot_test.pickle'), 'wb') as handle:
            pickle.dump(test_set, handle, protocol=2)

        with open(os.path.join('./', 'compacted_datasets', 'omniglot_label_encoder.pickle'), 'wb') as handle:
            pickle.dump(label_encoder, handle, protocol=2)
        with open(os.path.join('./', 'compacted_datasets', 'omniglot_label_decoder.pickle'), 'wb') as handle:
    ...
    def load_dataset(self, train, size):
        ...
        if train:
            with open(os.path.join('./', 'compacted_datasets', 'omniglot_train.pickle'), 'rb') as handle:
                data = pickle.load(handle)
        else:
            with open(os.path.join('./', 'compacted_datasets', 'omniglot_test.pickle'), 'rb') as handle:
                data = pickle.load(handle)
