...
def gmul(input):
    ...
    # output.shape: torch.Size([100, 12, 165])

    output = output.split(N, 1)
    output = torch.cat(output, 2) # output has size (bs, N, J*num_features)
    # output.shape: torch.Size([100, 6, 330])
...
class AdjMatCompute(nn.Module):
    ...
    def forward(self, x, W_id):
        ...
        # W_new.shape: torch.Size([10, 165, 6, 6])
...
class GNN_nl_omniglot(nn.Module):
    def __init__(self, args, input_features, nf, J):
        ...
        self.args = args
        self.input_features = input_features # emb_size + train_N_way
        self.nf = nf
        self.J = J

        self.num_layers = 2
        for i in range(self.num_layers):
            module_w = AdjMatCompute(self.input_features + int(nf / 2) * i, self.input_features + int(nf / 2) * i, operator='J2', activation='softmax', ratio=[2, 1.5, 1, 1], drop=False)
            module_l = GcBlock(self.input_features + int(nf / 2) * i, int(nf / 2), 2)
            self.add_module('layer_w{}'.format(i), module_w)
            self.add_module('layer_l{}'.format(i), module_l)

        self.w_comp_last = AdjMatCompute(self.input_features + int(self.nf / 2) * self.num_layers, self.input_features + int(self.nf / 2) * (self.num_layers - 1),
    ...
    def forward(self, nodes):
        ...
        for i in range(self.num_layers):
            # AdjMatCompute: message passing
            Wi = self._modules['layer_w{}'.format(i)](nodes, W_init)
            # GcBlock
            nodes_new = F.leaky_relu(self._modules['layer_l{}'.format(i)]([Wi, nodes])[1])
            nodes = torch.cat([nodes, nodes_new], 2)
...
class GNN_nl(nn.Module):
    def __init__(self, args, input_features, nf, J):
        ...
        for i in range(self.num_layers):
            if i == 0:
                module_w = AdjMatCompute(self.input_features, nf, operator='J2', activation='softmax', ratio=[2, 2, 1, 1])
                module_l = GcBlock(self.input_features, int(nf / 2), 2)
            else:
                module_w = AdjMatCompute(self.input_features + int(nf / 2) * i, nf, operator='J2', activation='softmax', ratio=[2, 2, 1, 1])
                module_l = GcBlock(self.input_features + int(nf / 2) * i, int(nf / 2), 2)
            self.add_module('layer_w{}'.format(i), module_w)
            self.add_module('layer_l{}'.format(i), module_l)

        self.w_comp_last = AdjMatCompute(self.input_features + int(self.nf / 2) * self.num_layers, nf, operator='J2', activation='softmax', ratio=[2, 2, 1, 1])
        self.layer_last = GcBlock(self.input_features + int(self.nf / 2) * self.num_layers, args.train_N_way, 2, bn_bool=False)
