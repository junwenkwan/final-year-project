...
class OmniglotNShot:
    ...
    def __init__(self, root, batchsz, n_way, k_shot, k_query, imgsz):
        ...
        self.resize = imgsz # 28
        if not os.path.isfile(os.path.join('./data', 'omniglot.npy')):
            ...
            print('data shape:', self.x.shape)  # [1623, 20, 1, 28, 28]
            
            temp = []  # Free memory

            # save all dataset into npy file.
            np.save(os.path.join('./data', 'omniglot.npy'), self.x)
            print('write into omniglot.npy.')
        else:
            # if data.npy exists, just load it.
            self.x = np.load(os.path.join('./data', 'omniglot.npy'))
            print('Loaded from omniglot.npy')
        ...
        self.k_query = k_query  # k query: 15
    ...
    def load_data_cache(self, data_pack):
        ...
        setsz = self.k_shot * self.n_way # 1*20
        querysz = self.k_query * self.n_way # 15*20
        data_cache = []
